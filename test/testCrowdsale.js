var ZTaylorCoinCrowdsale = artifacts.require('ZTaylorCoinCrowdsale');
var ZTaylorCoin = artifacts.require('ZTaylorCoin');

contract('ZTaylorCoinCrowdsale', function(accounts) {
  it('should deploy the token and store the address', function(done) {
    ZTaylorCoinCrowdsale.deployed().then(async function(instance) {
  ...
  it('one ETH should buy 5 ZTaylorCoins in PreICO', function(done) {
    ZTaylorCoinCrowdsale.deployed().then(async function(instance) {
      ...
      const ztaylorCoin = ZTaylorCoin.at(tokenAddress);
      const tokenAmount = await ztaylorCoin.balanceOf(accounts[7]);
      assert.equal(
  ...
  it('one ETH should buy 2 ZTaylorCoins in ICO', function(done) {
    ZTaylorCoinCrowdsale.deployed().then(async function(instance) {
      const data = await instance.sendTransaction({
        from: accounts[2],
      ...
      const tokenAddress = await instance.token.call();
      const ztaylorCoin = ZTaylorCoin.at(tokenAddress);
      const tokenAmount = await ztaylorCoin.balanceOf(accounts[2]);
      assert.equal(
        tokenAmount.toNumber(),
        3000000000000000000,
